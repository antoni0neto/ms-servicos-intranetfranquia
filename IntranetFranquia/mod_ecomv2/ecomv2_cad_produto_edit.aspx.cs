...
{
    public partial class ecomv2_cad_produto_edit : System.Web.UI.Page
    {
        ...
        const string PRODCATMAG = "PRODCATMAGV2";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (p != null)
                {
                    ...
                    string codCategoria = gCodCategoria;
                    ...
                    hidGrupoProduto.Value = grupoProduto;
                    ...
                    CarregarCategoriaMag(griffe, grupoProduto);
                    ...
                    if (produtoMag != null)
                    {
                        ...
                        cbCadastrarMKTPlace.Checked = produtoMag.CAD_MKTPLACE;
                        ...
                        if (produtoMag.FOTO_HOVER_POS == produtoMag.FOTO_LOOK_POS)
                            rbHoverLook.Checked = true;
                        else if (produtoMag.FOTO_HOVER_POS == produtoMag.FOTO_FRENTE_CAB_POS)
                            rbHoverFrenteCabeca.Checked = true;
                        else if (produtoMag.FOTO_HOVER_POS == produtoMag.FOTO_FRENTE_SEMCAB_POS)
                            rbHoverFrenteSemCabeca.Checked = true;
                        else if (produtoMag.FOTO_HOVER_POS == produtoMag.FOTO_COSTAS_POS)
                            rbHoverCostas.Checked = true;
                        else if (produtoMag.FOTO_HOVER_POS == produtoMag.FOTO_DETALHE_POS)
                            rbHoverDetalhe.Checked = true;
                        else if (produtoMag.FOTO_HOVER_POS == produtoMag.FOTO_LADO_POS)
                        ...
                        CarregarCategorias();
                        CarregarBlocos(produtoMag.CODIGO);
                        CarregarLojas(produtoMag.SKU);
                    ...
                    else
                    {
                        ...
                        txtMetaKeyword.Text = "roupas online, moda feminina, moda masculina, roupas em promoção, handbook, " + grupoProduto.ToLower().Replace("basic", "camiseta") + ", " + c.ToLower() + ", " + griffe.ToLower() + ", fashion, roupas, estilo, looks, moda, verão, primavera";
        ...
        private void CarregarCategoriaMag(string griffe = "", string grupoProduto = "")
        {
            ...
            var categoriaPrincipal = ddlGrupoMagento.SelectedItem.Text.Trim().ToLower();
            if (categoriaPrincipal.Contains("out"))
                catMag = catMag.Where(x => x.GRUPO.ToLower().Contains("out")).ToList();
            else
                catMag = catMag.Where(x => !x.GRUPO.ToLower().Contains("out")).ToList();
        ...
        {
            CarregarCategoriaMag(hidGriffe.Value, hidGrupoProduto.Value);
        ...
        private bool SalvarProdutoConfig()
        {
            ...
            produtoMag.URL_PRODUTO = produtoMag.URL_PRODUTO.Replace(".", "");
            ...
            if (rbHoverLook.Checked)
                produtoMag.FOTO_HOVER_POS = Convert.ToInt32(ddlOrdemLook.SelectedValue);
            else if (rbHoverFrenteCabeca.Checked)
                produtoMag.FOTO_HOVER_POS = Convert.ToInt32(ddlOrdemFrenteCabeca.SelectedValue);
            else if (rbHoverFrenteSemCabeca.Checked)
                produtoMag.FOTO_HOVER_POS = Convert.ToInt32(ddlOrdemFrenteSemCabeca.SelectedValue);
            else if (rbHoverCostas.Checked)
                produtoMag.FOTO_HOVER_POS = Convert.ToInt32(ddlOrdemCostas.SelectedValue);
            else if (rbHoverDetalhe.Checked)
                produtoMag.FOTO_HOVER_POS = Convert.ToInt32(ddlOrdemDetalhe.SelectedValue);
            else if (rbHoverLado.Checked)
                produtoMag.FOTO_HOVER_POS = Convert.ToInt32(ddlOrdemLado.SelectedValue);
        ...
        protected void btExcluirCategoriaProd_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    var ecomProdutoOrdem = eController.ObterBlocoProdutoOrdemPorCodigoEcomGrupoProduto(codigoGrupoProduto)
                                                .Where(x => x.ECOM_PRODUTO == codigoEcom).FirstOrDefault();
                    if (ecomProdutoOrdem != null)
                    {
                        eController.ExcluirBlocoProdutoOrdem(ecomProdutoOrdem.CODIGO);
                        CarregarBlocos(codigoEcom);
                    }
                }
        ...
        private void CarregarBlocos(int codigoEcom)
        {
            var blocos = eController.ObterBlocoProdutoOrdemPorCodigoEcomProduto(codigoEcom);
        ...
        private void CarregarLojas(string sku)
        {
            var blocos = eController.ObterWebSiteSku(sku);
            gvLojas.DataSource = blocos;
            gvLojas.DataBind();
        }
        protected void gvLojas_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if (e.Row.DataItem != null)
                {
                    ECOM_WEBSITE_SKU sku = e.Row.DataItem as ECOM_WEBSITE_SKU;
                }
            }
        }
        ...
        private void SalvarProdutoMagento()
        {
            ...
            foreach (var p in produtos)
            {
                ...
                mag.AtualizarProduto(p, updateImage);
                updateImage = false;
            }
