...
{
    public partial class facc_resumo_timev2 : System.Web.UI.Page
    {
        FaccaoController faccController = new FaccaoController();
        ProducaoController prodController = new ProducaoController();

        List<SP_OBTER_FACCAO_TIMEV2Result> faccaoResumoAux = new List<SP_OBTER_FACCAO_TIMEV2Result>();
        ...
        private List<SP_OBTER_FACCAO_TIMEV2Result> ObterFaccaoTime()
        {
            var listaRetorno = new List<SP_OBTER_FACCAO_TIMEV2Result>();
            ...
            var resumoHBF = faccController.ObterFaccaoTimeV2(ddlColecao.SelectedValue.Trim(), numeroHB, mostruario, ddlFornecedor.SelectedValue.Trim(), ddlFornecedorSub.SelectedValue.Trim(), prodServico, ddlStatus.SelectedValue);

            if (ddlAtraso.SelectedValue != "")
            {
                if (ddlAtraso.SelectedValue == "S")
                    resumoHBF = resumoHBF.Where(p => p.ATRASO_EM_DIAS > 0).ToList();
                else
                    resumoHBF = resumoHBF.Where(p => p.ATRASO_EM_DIAS <= 0).ToList();
            }
            ...
            faccaoResumoAux = faccaoResumoAux.GroupBy(p => new { COLECAO = p.COLECAO, HB = p.HB, MOSTRUARIO = p.MOSTRUARIO, QTDE_ATACADO = p.QTDE_ATACADO, QTDE_VAREJO = p.QTDE_VAREJO }).Select(x => new SP_OBTER_FACCAO_TIMEV2Result
            {
        ...
        private List<SP_OBTER_FACCAO_TIME_ENCAIXEResult> ObterFaccaoTimeEncaixe()
        {
            ...
            faccaoEncaixeAux = faccaoEncaixeAux.GroupBy(p => new { COLECAO = p.COLECAO, HB = p.HB, MOSTRUARIO = p.MOSTRUARIO, QTDE_ATACADO = p.QTDE_ATACADO, QTDE_VAREJO = p.QTDE_VAREJO }).Select(x => new SP_OBTER_FACCAO_TIME_ENCAIXEResult
            {
                COLECAO = x.Key.COLECAO,
                HB = x.Key.HB,
                MOSTRUARIO = x.Key.MOSTRUARIO,
        ...
        protected void btLimpar_Click(object sender, EventArgs e)
        {
            ...
            gvFaccaoHB.DataSource = new List<SP_OBTER_FACCAO_TIMEV2Result>();
            gvFaccaoHB.DataBind();
        ...
        protected void gvFaccaoHB_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                {
                    SP_OBTER_FACCAO_TIMEV2Result faccaoHB = e.Row.DataItem as SP_OBTER_FACCAO_TIMEV2Result;
        ...
        {
            IEnumerable<SP_OBTER_FACCAO_TIMEV2Result> _faccResumo = ObterFaccaoTime();
