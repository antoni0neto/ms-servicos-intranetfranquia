...
{
    public partial class mkt_gin_natal2020 : System.Web.UI.Page
    {
        LojaController lojaController = new LojaController();

        string semana = "Início da Pontuação - 01/12/2020";
        ...
        {
            var gincana = lojaController.ObterGincanaNAT2020();
            if (gincana != null)
            {
        ...
        protected void gvPrincipal_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                {
                    SP_OBTER_GINCANA_NAT2020Result gin = e.Row.DataItem as SP_OBTER_GINCANA_NAT2020Result;

                    if (gin != null)
                    {

                        if (semana == "Início da Pontuação - 01/12/2020")
                        {
