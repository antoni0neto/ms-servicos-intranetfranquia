...
{
    public partial class pacab_pedido_produto_conv2 : System.Web.UI.Page
    {
        DesenvolvimentoController desenvController = new DesenvolvimentoController();
        ProducaoController prodController = new ProducaoController();
        BaseController baseController = new BaseController();

        const string PROD_ACAB_PEDCOMPRA_PROD_CON = "PROD_ACAB_PEDCOMPRA_PROD_CON";
        ...
        {
            var produtoAcabado = ObterProdutoAcabadoProdutoPed(colecao, desenvOrigem, grupo, produto, filial, fornecedor);

            gvProdutoAcabado.DataSource = produtoAcabado;
            gvProdutoAcabado.DataBind();

            Session[PROD_ACAB_PEDCOMPRA_PROD_CON] = produtoAcabado;
        }
        protected void gvProdutoAcabado_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                {
                    SP_OBTER_PRODUTO_ACABADO_PRODUTO_PEDResult produtoAcabado = e.Row.DataItem as SP_OBTER_PRODUTO_ACABADO_PRODUTO_PEDResult;
                    ...
                    if (produtoAcabado != null)
                    {
                        ...
                        if (produtoAcabado.FOTO1 != null && produtoAcabado.FOTO1.Trim() != "")
                        {
                            ...
                            if (gvFoto != null)
                            {
                                ...
                                _fotoProduto.Add(new DESENV_PRODUTO { CODIGO = produtoAcabado.CODIGO, FOTO = produtoAcabado.FOTO1, FOTO2 = produtoAcabado.FOTO2 });
        ...
        {
            if (Session[PROD_ACAB_PEDCOMPRA_PROD_CON] != null)
            {
                IEnumerable<SP_OBTER_PRODUTO_ACABADO_PRODUTO_PEDResult> produtoAcabado = (IEnumerable<SP_OBTER_PRODUTO_ACABADO_PRODUTO_PEDResult>)Session[PROD_ACAB_PEDCOMPRA_PROD_CON];
        ...
        protected void gvFoto_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if (e.Row.DataItem != null)
                {
                    DESENV_PRODUTO produto = e.Row.DataItem as DESENV_PRODUTO;
                    if (produto != null)
                    {
                        ...
                        if (_imgFotoPeca != null)
                            _imgFotoPeca.ImageUrl = produto.FOTO;
        ...
        private List<SP_OBTER_PRODUTO_ACABADO_PRODUTO_PEDResult> ObterProdutoAcabadoProdutoPed(string colecao, int? desenvOrigem, string grupo, string produto, string filial, string fornecedor)
        {
            var produtoAcabado = desenvController.ObterProdutoAcabadoProdutoPed(colecao, desenvOrigem, grupo, produto, filial, fornecedor);
